...
def corp_alliance_info(request, character):
    headers = {
    "Accept-Language": "",
    "If-None-Match": "",
    ...
    response_char = requests.get(f'{settings.EVE_ESI_URL}/characters/{character["CharacterID"]}/', headers=headers)
    if response_char.status_code != 200:
        messages.error(request, "Failed to retrieve character information from EVE ESI.")
    ...
    if 'corporation_id' in data_char:
        ...
        if response_corp.status_code != 200:
            messages.error(request, "Failed to retrieve corporation information from EVE ESI.")
            return redirect('index')
    ...
    if 'alliance_id' in data_char:
        ...
        if response_alliance.status_code != 200:
            messages.error(request, "Failed to retrieve alliance information from EVE ESI.")
            return redirect('index')
...
def wallet_info(request, character):
    headers = {
    "Accept-Language": "",
    "If-None-Match": "",
    "X-Compatibility-Date": "2020-01-01",
    ...
    if response_wallet.status_code != 200:
        messages.error(request, "Failed to retrieve wallet information from EVE ESI.")
        return redirect('index')
